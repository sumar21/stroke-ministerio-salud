...
import { Plus, Edit2, Trash2, User as UserIcon, Shield, Key, Radio } from 'lucide-react';
...
export function UsersView() {
  ...
  const handleEdit = (user: User) => {
    setCurrentUser({
      ...user,
      operativeCenterId: user.role === 'AMBULANCE' ? (user.operativeCenterId || NONE) : undefined,
    });
    setIsEditing(true);
  };
  ...
  const handleSave = () => {
    if (currentUser && currentUser.id) {
      const toSave: User = {
        ...(currentUser as User),
        operativeCenterId: currentUser.operativeCenterId === NONE ? undefined : currentUser.operativeCenterId,
      };
      const exists = users.find(u => u.id === toSave.id);
      if (exists) {
        setUsers(users.map(u => u.id === toSave.id ? toSave : u));
      } else {
        setUsers([...users, toSave]);
      }
      setIsEditing(false);
      setCurrentUser(null);
    }
  ...
  const handleRoleChange = (role: string) => {
    setCurrentUser({
      ...currentUser!,
      role,
      operativeCenterId: role === 'AMBULANCE' ? (currentUser?.operativeCenterId || NONE) : undefined,
    });
  };
  ...
  return (
    <div className="flex-1 w-full p-4 md:p-6 space-y-6 animate-in fade-in duration-500">
      ...
      <ConfirmModal
        isOpen={isDeleting}
        onClose={() => { setIsDeleting(false); setUserToDelete(null); }}
        ...
        <div className="space-y-4">
          <div className="grid grid-cols-1 gap-4">
            ...
            <div className="space-y-1.5">
              ...
              <Select value={currentUser?.role || 'AMBULANCE'} onValueChange={handleRoleChange}>
                <SelectTrigger>
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
                  <SelectItem value="AMBULANCE">Ambulancia (Carga)</SelectItem>
                  <SelectItem value="COORDINATION">Coordinación Centro Stroke</SelectItem>
                  <SelectItem value="DINESA">DINESA (Monitor)</SelectItem>
                  <SelectItem value="ADMIN">Administrador</SelectItem>
                  <SelectItem value="HOSPITAL">Hospital (Recepción)</SelectItem>
                </SelectContent>
              </Select>
            </div>
            {isAmbulance && (
              <div className="space-y-1.5">
                <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest flex items-center gap-1.5">
                  <Radio className="w-3 h-3" /> Centro Operativo
                </label>
                {mockOperativeCenters.length === 0 ? (
                  <p className="text-sm text-slate-400 font-medium px-3 py-2 bg-slate-50 rounded-xl border border-slate-200">
                    Sin centros operativos registrados
                  </p>
                ) : (
                  <Select
                    value={currentUser?.operativeCenterId || NONE}
                    onValueChange={v => setCurrentUser({ ...currentUser!, operativeCenterId: v })}
                  >
                    <SelectTrigger>
                      <SelectValue />
                    </SelectTrigger>
                ...
                <tr key={user.id} className="hover:bg-slate-50/80 transition-colors group">
                  <td className="px-6 py-5">
                    <div className="flex items-center gap-3">
                      <div className="w-9 h-9 rounded-xl bg-slate-100 text-slate-600 flex items-center justify-center text-xs font-black uppercase border border-slate-200 group-hover:bg-brand-navy/10 group-hover:text-brand-navy group-hover:border-brand-navy/20 transition-colors">
                  ...
                  <td className="px-6 py-5">
                    ...
                    {user.role === 'AMBULANCE' && user.operativeCenterId && (
                      <div className="flex items-center gap-1 mt-0.5 text-[10px] text-brand-navy/70 font-semibold">
                        <Radio className="w-2.5 h-2.5" />
                        {mockOperativeCenters.find(oc => oc.id === user.operativeCenterId)?.name ?? user.operativeCenterId}
                      </div>
                    )}
                  </td>
                  <td className="px-6 py-5">
                  ...
                  <td className="px-6 py-5 text-right">
                    <div className="flex justify-end gap-1">
                      <Button variant="ghost" size="sm" onClick={() => handleEdit(user)} className="h-8 w-8 p-0 rounded-lg text-brand-navy hover:text-brand-navy hover:bg-brand-navy/10">
