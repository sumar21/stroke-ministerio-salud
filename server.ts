...
// Helper to build patient info table rows (reused across email templates)
function buildPatientTableRows(patientData: any) {
  return `
    <tr>
      <td style="padding: 10px 0; border-bottom: 1px solid #e2e8f0; width: 40%; color: #64748b; font-weight: 500;">Nombre</td>
      <td style="padding: 10px 0; border-bottom: 1px solid #e2e8f0; font-weight: 600; color: #0f172a;">${patientData.name || 'N/A'}</td>
    </tr>
    <tr>
      <td style="padding: 10px 0; border-bottom: 1px solid #e2e8f0; color: #64748b; font-weight: 500;">DNI</td>
      <td style="padding: 10px 0; border-bottom: 1px solid #e2e8f0; font-weight: 600; color: #0f172a;">${patientData.id || 'N/A'}</td>
    </tr>
    <tr>
      <td style="padding: 10px 0; border-bottom: 1px solid #e2e8f0; color: #64748b; font-weight: 500;">Edad / Sexo</td>
      <td style="padding: 10px 0; border-bottom: 1px solid #e2e8f0; font-weight: 600; color: #0f172a;">${patientData.age || 'N/A'} / ${patientData.sex || 'N/A'}</td>
    </tr>
    <tr>
      <td style="padding: 10px 0; border-bottom: 1px solid #e2e8f0; color: #64748b; font-weight: 500;">Cobertura</td>
      <td style="padding: 10px 0; border-bottom: 1px solid #e2e8f0; font-weight: 600; color: #0f172a;">${patientData.coverage || 'N/A'}</td>
    </tr>
    <tr>
      <td style="padding: 10px 0; border-bottom: 1px solid #e2e8f0; color: #64748b; font-weight: 500;">Inicio de Síntomas</td>
      <td style="padding: 10px 0; border-bottom: 1px solid #e2e8f0; font-weight: 600; color: #ef4444;">${patientData.symptomOnsetTime || 'N/A'}</td>
    </tr>
    <tr>
      <td style="padding: 10px 0; border-bottom: 1px solid #e2e8f0; color: #64748b; font-weight: 500;">Contacto</td>
      <td style="padding: 10px 0; border-bottom: 1px solid #e2e8f0; font-weight: 600; color: #0f172a;">${patientData.contactInfo || 'N/A'}</td>
    </tr>
  `;
}
...
const NOTIFICATION_RECIPIENTS = [
  { role: "DINESA", email: "mazurmendi@msal.gov.ar", cc: "" },
  { role: "Centro Coordinador SAME", email: "lgaggino@msal.gov.ar", cc: "" },
  { role: "Centro Stroke", email: "dmassaragian@msal.gov.ar", cc: "" }
];

// POST /api/confirm-hospital
// Called when DINESA confirms the same hospital that was pre-assigned (or assigns a hospital from scratch).
app.post('/api/confirm-hospital', async (req, res) => {
  const { caseId, patientData, confirmedHospitalId, etaText } = req.body;

  const confirmedHospital = mockHospitals.find(h => h.id === confirmedHospitalId);
  const hospitalName = confirmedHospital?.name ?? 'Hospital desconocido';
  const hospitalAddress = confirmedHospital?.location.address ?? 'Dirección no disponible';
  const etaDisplay = etaText ? ` (ETA: ${etaText})` : '';
  ...
  const html = `
    <div style="font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; max-width: 680px; margin: 0 auto; color: #334155; border: 1px solid #e2e8f0; border-radius: 8px; overflow: hidden;">
      <div style="background-color: #10b981; padding: 24px; color: white; text-align: center;">
        <h1 style="margin: 0; font-size: 22px; font-weight: 700; letter-spacing: 0.5px;">✔ DERIVACIÓN CONFIRMADA – CÓDIGO ACV</h1>
      ...
      <div style="padding: 24px; background-color: #f8fafc;">
        ...
        <table style="width: 100%; border-collapse: collapse; margin-top: 12px;">
          ${buildPatientTableRows(patientData)}
        </table>

        ${patientData.symptoms && patientData.symptoms.length ? `
          <h3 style="margin-top: 22px; color: #0f172a; font-size: 15px;">Síntomas Detectados</h3>
          <ul style="margin-top: 8px; padding-left: 20px; color: #0f172a;">
            ${patientData.symptoms.map((s: string) => `<li style="margin-bottom: 4px;">${s}</li>`).join('')}
          </ul>
        ` : ''}
        ...
        <p style="margin-top: 8px; color: #0f172a; background-color: #d1fae5; padding: 12px; border-radius: 6px; border-left: 4px solid #10b981;">
          <strong>${hospitalName}</strong><br/><span style="font-size: 13px; color: #374151;">${hospitalAddress}</span>
        </p>
      </div>
      ${EMAIL_FOOTER}
    </div>
...
// POST /api/reassign-hospital
// Called when DINESA assigns a DIFFERENT hospital than the pre-assigned one.
// Sends: (1) cancellation email to the original hospital, (2) new assignment email to the new hospital.
app.post('/api/reassign-hospital', async (req, res) => {
  ...
  // --- Email 1: Cancellation to the original (cancelled) hospital ---
  const cancellationHtml = `
    <div style="font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; max-width: 680px; margin: 0 auto; color: #334155; border: 1px solid #e2e8f0; border-radius: 8px; overflow: hidden;">
      <div style="background-color: #f59e0b; padding: 24px; color: white; text-align: center;">
        <h1 style="margin: 0; font-size: 22px; font-weight: 700; letter-spacing: 0.5px;">⚠ CANCELACIÓN DE DERIVACIÓN – CÓDIGO ACV</h1>
      ...
      <div style="padding: 24px; background-color: #f8fafc;">
        <div style="background-color: #fef3c7; border: 1px solid #fde68a; border-radius: 6px; padding: 14px 18px; margin-bottom: 20px;">
          ...
          <p style="margin: 6px 0 0 0; color: #b45309; font-size: 13px;">
            La derivación del paciente a <strong>${cancelledName}</strong> ha sido <strong>cancelada</strong> por decisión operativa de DINESA.<br/>
            El paciente ha sido redirigido a otro centro asistencial. No se requiere preparación adicional para este caso.
          </p>
        </div>
  ...
  // --- Email 2: New assignment to the new hospital (same look as submit-acv) ---
  const newAssignmentHtml = `
    <div style="font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; max-width: 680px; margin: 0 auto; color: #334155; border: 1px solid #e2e8f0; border-radius: 8px; overflow: hidden;">
      <div style="background-color: #ef4444; padding: 24px; color: white; text-align: center;">
        <h1 style="margin: 0; font-size: 22px; font-weight: 700; letter-spacing: 0.5px;">🚨 ALERTA: CÓDIGO ACV – DERIVACIÓN CONFIRMADA</h1>
      ...
      <div style="padding: 24px; background-color: #f8fafc;">
        <div style="background-color: #fee2e2; border: 1px solid #fca5a5; border-radius: 6px; padding: 14px 18px; margin-bottom: 20px;">
          ...
          <p style="margin: 6px 0 0 0; color: #991b1b; font-size: 13px;">
            DINESA ha confirmado la derivación de un paciente con <strong>Código ACV</strong> a su institución.<br/>
            Se solicita activación inmediata del equipo de stroke y preparación de guardia neurológica.
          </p>
        </div>
        ...
        ${patientData.clinicalObservations ? `
          <h3 style="margin-top: 22px; color: #0f172a; font-size: 15px;">Observaciones Clínicas</h3>
          <p style="margin-top: 8px; color: #0f172a; background-color: #f1f5f9; padding: 12px; border-radius: 6px; border-left: 4px solid #94a3b8;">${patientData.clinicalObservations}</p>
        ` : ''}

        <h3 style="margin-top: 22px; color: #0f172a; font-size: 15px;">Hospital Confirmado como Destino</h3>
        <p style="margin-top: 8px; color: #0f172a; background-color: #fee2e2; padding: 12px; border-radius: 6px; border-left: 4px solid #ef4444;">
          <strong>${newHospitalName}</strong><br/><span style="font-size: 13px; color: #374151;">${newHospitalAddress}</span>
        </p>
      </div>
      ${EMAIL_FOOTER}
    </div>
